...
public class MemberCommands extends RegionCommandsBase {
    ...
    public void addMember(CommandContext args, Actor sender) throws CommandException {
        ...
        final String description = String.format("正在向世界 '%s' 的区域 '%s' 添加成员", world.getName(), region.getId());
        AsyncCommandBuilder.wrap(resolver, sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .onSuccess(String.format("区域 '%s' 已更新新成员。", region.getId()), region.getMembers()::addAll)
                .onFailure("无法添加新成员", worldGuard.getExceptionConverter())
    ...
    public void addOwner(CommandContext args, Actor sender) throws CommandException {
        ...
        final String description = String.format("正在向世界 '%s' 的区域 '%s' 添加所有者", world.getName(), region.getId());
        AsyncCommandBuilder.wrap(checkedAddOwners(sender, manager, region, world, resolver), sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .onSuccess(String.format("区域 '%s' 已更新新所有者。", region.getId()), region.getOwners()::addAll)
                .onFailure("无法添加新所有者", worldGuard.getExceptionConverter())
                .buildAndExec(worldGuard.getExecutorService());
    }
            ...
            if (sender instanceof LocalPlayer) {
                ...
                if (owners.contains(player) && !sender.hasPermission("worldguard.region.unlimited")) {
                    ...
                    if (maxRegionCount >= 0 && manager.getRegionCountOfPlayer(player)
                            >= maxRegionCount) {
                        throw new CommandException("你已经拥有了最大允许数量的区域。");
                    }
                }
            }
    ...
    public void removeMember(CommandContext args, Actor sender) throws CommandException {
        ...
        if (args.hasFlag('a')) {
            ...
            if (args.argsLength() < 2) {
                throw new CommandException("请列出要移除的名称，或使用 -a 全部移除。");
            }
        ...
        final String description = String.format("正在从世界 '%s' 的区域 '%s' 移除成员", world.getName(), region.getId());
        AsyncCommandBuilder.wrap(callable, sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .sendMessageAfterDelay("（请稍候...正在查询玩家名称...）")
                .onSuccess(String.format("区域 '%s' 已更新，成员已移除。", region.getId()), region.getMembers()::removeAll)
                .onFailure("无法移除成员", worldGuard.getExceptionConverter())
                .buildAndExec(worldGuard.getExecutorService());
    }
    ...
    public void removeOwner(CommandContext args, Actor sender) throws CommandException {
        ...
        final String description = String.format("正在从世界 '%s' 的区域 '%s' 移除所有者", world.getName(), region.getId());
        AsyncCommandBuilder.wrap(callable, sender)
                .registerWithSupervisor(worldGuard.getSupervisor(), description)
                .sendMessageAfterDelay("（请稍候...正在查询玩家名称...）")
                .onSuccess(String.format("区域 '%s' 已更新，所有者已移除。", region.getId()), region.getOwners()::removeAll)
                .onFailure("无法移除所有者", worldGuard.getExceptionConverter())
                .buildAndExec(worldGuard.getExecutorService());
    }
}
